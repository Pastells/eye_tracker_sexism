...
La comparació tripartita entre atenció humana, anotacions de segments i atribucions del model ens ha permès caracteritzar fins a quin punt aquestes fonts convergeixen i on divergeixen.

De l'anàlisi en destaquem quatre resultats principals.
En primer lloc, observem el que anomenem la paradoxa de BETO: el model amb menor precisió (62,2\%) presenta les distribucions de prominència més properes a l'atenció humana (SD: 2,99--3,46).
...
En segon lloc, LRP és el mètode d'interpretabilitat que produeix les distribucions més properes a l'atenció humana en tots els models (SD: 2,99--4,12), possiblement per la seva naturalesa de decomposició exacta del gradient, que preserva la informació contributiva del model.
...
Cal destacar que la nostra expectativa inicial---que els mètodes basats en gradients estarien més alineats amb l'atenció humana---només es confirma parcialment: tot i que LRP produeix les SD més baixes, IG mostra les SD més altes, malgrat que ambdós mètodes es basen en gradients.
...
Respecte al \textbf{tercer objectiu}, la comparació tripartita (model--segments, model--humà, humà--segments) revela divergències substancials.
...
En canvi, la divergència entre el model i les anotacions és alta (0,78--0,84), cosa que indica que els models distribueixen l'atenció de manera substancialment diferent.
...
La superposició de \textit{hotspots} entre atenció humana i segments anotats és alta (60,7\%--71,9\%), però la distribució de prominència del model divergeix de l'atenció humana (JS: 0,78--0,84).
...
\noindent La hipòtesi es confirma parcialment.
Les correlacions de Spearman són properes a zero per a totes les categories, però les divergències varien: els estereotips (JS 0,23) i l'objectificació (JS 0,24) presenten les divergències més baixes amb l'atenció humana, mentre que la desigualtat ideològica (JS 0,50) és la més alta.
Les categories més explícites (estereotips, objectificació) mostren, doncs, un alineament millor, consistent amb la hipòtesi.
...
La comparació tripartita mostra que cap font per si sola no ofereix una visió completa del fenomen.
