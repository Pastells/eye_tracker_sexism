Aquest capítol presenta l'anàlisi de les dades de seguiment ocular---fixacions i regressions---i la seva relació amb les anotacions del \textit{gold standard} i les explicacions dels models de llenguatge. També analitza les decisions dels participants durant l'experiment.
...
% TODO: fer aquesta anàlisi més detallada
...
La divergència JS mitjana és 0,25, fet que apunta a una separació substancial entre les dues distribucions.
% TODO: mirar figura
...
Els punts focals amb les puntuacions z més altes són:
...
N'extraiem la prominència amb quatre mètodes d'interpretabilitat: Saliency, Input$\times$Gradient, Integrated Gradients (IG) i Layer-wise Relevance Propagation (LRP).
...
En segon lloc, a la \autoref{tab:model_vs_human_dist} comparem les distribucions de prominència dels models amb l'atenció humana (TFD).
...
La JS entre els models i l'atenció humana (0,77--0,84) és pràcticament la mateixa que entre els models i els segments (0,78--0,84).
...
\end{table}
\fi
La divergència JS més baixa és la que s'observa entre la TFD i les anotacions de segments (0,12). Això indica que els anotadors i els lectors comparteixen una visió similar del que és sexista.
La divergència més alta és la que es dona entre els models i les anotacions de segments (0,78--0,84), cosa que indica que els models de llenguatge distribueixen la prominència de manera substancialment diferent de les persones.
